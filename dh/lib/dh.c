...
int get_dh_params(DH *dh) {
  ...
  return DH_set0_pqg(dh, BN_bin2bn(p, sizeof(p), NULL), NULL, BN_bin2bn(g, sizeof(g), NULL));
}

int crypto_dh_keypair(DH *dh) {
  if (get_dh_params(dh) == 0)
    return -1;
...
int crypto_dh_enc(unsigned char *k, DH *dh, const BIGNUM *pk) {
  ...
  if (DH_compute_key(secret, pk, dh) == -1) {
    return -1;
  }
